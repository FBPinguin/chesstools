...
<span>
<label for="message" class="block mb-2 text-sm font-medium text-gray-900">Chessbase</label>
<textarea id="message" rows="4" class="p-2.5 w-full h-screen text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="PGN" bind:value={chessbasepgn} onchange={() => {convertertolichess(chessbasepgn)}}></textarea>
</span>

<span class="place-self-center grid grid-rows-2">
<p class="block mb-2 text-sm font-medium text-gray-900 pl-4"><strong>This is a converter for Chessbase PGN to lichess PGN.</strong> Because lichess uses a different (better) PGN format than chessbase the time you can add in chessbase doesnt get added in lichess. Therefore this converter! <span class="italic">ps. the converter from lichess to chessbase is currently broken :/</span></p>
...
<span>
<label for="message" class="block mb-2 text-sm font-medium text-gray-900">Lichess</label>
<textarea id="message" rows="4" class="p-2.5 w-full h-screen text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="PGN" bind:value={lichesspgn} onchange={() => {convertertochessbase(lichesspgn)}}></textarea>
</span>
</span>